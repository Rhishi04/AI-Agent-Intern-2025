...
from .config import DYNAMIC_FIELD_MAPPINGS, PipelineConfig
import os
...
class InconsistencyPipeline:
    ...
    def _save_findings_to_csv(self, findings_df: pd.DataFrame):
        ...
        filename = f"{self.output_dir}/inconsistency_findings_{timestamp}.csv"
    ...
    def run_pipeline(self) -> List[Dict]:
        ...
        if any(not df.empty for tables in self.systems_data.values() for df in tables.values()):
            consistency_checker = CrossSystemConsistencyChecker(
                self.systems_data, 
                self.config.systems, 
                DYNAMIC_FIELD_MAPPINGS,  # Use dynamic field mappings
                batch_id,
